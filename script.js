...
class Live {
    ...
    constructor() {
      ...
      this.y = 0;
      this.id = 0;
      this.generateRefLife(++lives_number);
    }
    ...
    generateRefLife(nr) {
        ...
        this.ref.style.width = `${this.width}px`;
        this.ref.style.height = `${this.height}px`;
        this.ref.style.content = `url(./heart.png)`;
        this.ref.id = nr;
        this.id = nr;
        document.getElementById("lifes").appendChild(this.ref);
      }
...
class LiveFactory {
    ...
    destroyLive(x) {
      console.log(x+ 'Live Destroyed');
      
      this.lives = this.lives.filter((life) => {
          console.log(life.id);
        if (life.id ===x) {
            console.log("Live Id ", life.id);
            life.removeRef();
          return false;
        }
        else
        console.log("Not have to delete the life");
...
const lifesFactory = new LiveFactory();

lifesFactory.createLive();
lifesFactory.createLive();
lifesFactory.createLive();
...
function game_lifes(){
    ...
    if(collisionDetection(player, obstacleFactory.obstacles)){
        lifesFactory.destroyLive(lives_number);
        clearInterval(gameLoop);
        // if the player collide with any of the obstacles we need to close the game loops, alert the user and refresh the game
        while (collisionDetection(player, obstacleFactory.obstacles)) {
        obstacleFactory.moveObstacles();
        
        }

        lives_number--;
        // console.log(lives_number);

        if(lives_number>0)

            gameLoop = setInterval(game_lifes, 50);
        else{
            alert("You hit the last obstacle. No live available for you from 3. GAME OVER ! ");
